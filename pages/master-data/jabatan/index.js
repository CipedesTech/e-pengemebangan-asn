...
function MasterDataJabatan() {
  ...
  const onCreate = () => {
    router.push('/master-data/jabatan/create');
  };

  const onEdit = (id) => {
    router.push(`/master-data/jabatan/${id}/edit`);
  };
...
MasterDataJabatan.getLayout = function getLayout(page) {
  return (
    <AppLayout title='Jabatan' key={1} extra={false} onTab={0}>
      {page}
    </AppLayout>
  );
};
